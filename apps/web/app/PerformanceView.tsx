...
export function PerformanceView({ performance }: { performance: PublicPerformanceData | null }) {
  return (
      ...
      <article className="dashboard-card">
        <div className="section-row">
          <div>
            ...
            <h2>Actividad acumulativa de asistencia y kills.</h2>
          </div>
          {performance ? (
            <div className="actions">
              <span className="status-badge">{performance.trackedBattles} battles acumuladas</span>
              {performance.lastUpdatedAt ? (
                <span className="status-badge">
                  Actualizado {new Date(performance.lastUpdatedAt).toLocaleString("es-ES")}
                </span>
              ) : null}
            </div>
          ) : null}
        ...
        <p className="lede">
          Esta vista acumula batallas procesadas y CTAs finalizadas. Cada nueva battle detectada
          alimenta las medias de Main y Bombs.
        </p>
      </article>

      {!performance ? (
        <article className="dashboard-card">
          <p className="empty">No se ha podido cargar el rendimiento ahora mismo.</p>
        </article>
      ) : (
        <>
          <div className="dashboard-grid">
            <article className="dashboard-card metric-card">
              ...
              <strong>{formatCompact(performance.attendance.averageCount)}</strong>
              <p>
                {formatPercent(performance.attendance.averagePercent)} sobre miembros totales en{" "}
                {performance.attendance.ctaCount} battles acumuladas.
              </p>
            </article>
            <article className="dashboard-card metric-card">
              <span className="card-label">Main Kills media</span>
              <strong>{formatCompact(performance.main.averageKills)}</strong>
              <p>{formatPercent(performance.main.sharePercent)} del total de kills guardadas.</p>
            </article>
            <article className="dashboard-card metric-card">
              <span className="card-label">Bomb Kills media</span>
              <strong>{formatCompact(performance.bombTotals.averageKills)}</strong>
              <p>{formatPercent(performance.bombTotals.sharePercent)} del total de kills guardadas.</p>
            </article>
          </div>
              ...
              >
                ...
                {performance.bombs.map((bomb) => (
                  <Fragment key={bomb.bombGroupName}>
                    <div className="battle-table-cell battle-name-cell">{bomb.bombGroupName}</div>
                    <div className="battle-table-cell">
                      <span className="battle-value red">{formatCompact(bomb.averageKills)}</span>
                    </div>
                    <div className="battle-table-cell">
                      <span className="battle-value gold">{formatPercent(bomb.sharePercent)}</span>
                    </div>
                  </Fragment>
                ))}
              </div>
